...
| Groups | four uint32: first index, index count, albedo texture index, flags (bit 0: drawn but not collided with — Source `SOLID_NONE` static props; bit 1: drawn alpha-blended by the texture alpha — `$alphatest` and `$translucent` materials; older readers ignore both) |
